 ...
 */
export function analyzeProject(tree: string[]): ProjectAnalysis {
  ...
  const hasPagesDir = tree.some((p) => 
    (p.startsWith("pages/") || p.startsWith("src/pages/")) && 
    !p.includes("api/")  // Don't count API routes
  );
  
  // Determine project type
  let type: ProjectAnalysis["type"];
 ...
 */
export function getFaviconDestination(
  analysis: ProjectAnalysis
): { iconPath: string; icoPath: string; pngPath: string } {
 ...
 */
export function getAnalysisSummary(analysis: ProjectAnalysis): string {
  const parts: string[] = [];
